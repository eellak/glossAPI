...
pub fn count_charsets(text: &str) -> CharsetCounts {
    ...
    for line in text.lines() {
        ...
        let whole_line_in_latex = latex_spans.iter().any(|span| {
            span.start <= line_start && line_end <= span.end
        });
        ...
        for ch in line_stripped.chars() {
            if stripped_equals_original {
                ...
                // the LaTeX spans.
                let in_latex = latex_spans.iter().any(|span| {
                    span.start <= byte_off && byte_off < span.end
                });
...
";
    ...
    fn moji_includes_latin_ext_a() {
        ...
        assert!(c.latin_ext_a >= 8,
                "expected ≥8 latin_ext_a, got {}", c.latin_ext_a);
        let r = CharsetRatios::from_counts(&c);
        assert!(r.moji_residue_ratio > 0.4,
                "expected >0.4 moji ratio, got {}", r.moji_residue_ratio);
    }
    ...
    fn moji_includes_cyrillic() {
        ...
        let r = CharsetRatios::from_counts(&c);
        assert!(r.moji_residue_ratio > 0.9,
    ...
    fn moji_excludes_legit_greek_punctuation() {
        ...
        let r = CharsetRatios::from_counts(&c);
        assert!(c.latin1_legit_extras >= 9,
                "expected ≥9 legit_extras, got {}", c.latin1_legit_extras);
        assert!(r.moji_residue_ratio < 0.05,
                "legit Greek punct leaked into moji: ratio={} (latin1_supp={}, legit_extras={})",
                r.moji_residue_ratio, c.latin1_supp, c.latin1_legit_extras);
    }
    ...
    fn moji_still_catches_actual_mojibake_despite_legit_subtraction() {
        ...
        let r = CharsetRatios::from_counts(&c);
        assert_eq!(c.latin1_legit_extras, 0,
                   "no legit-extras should be in codepage-mojibake sample");
        assert!(r.moji_residue_ratio > 0.85,
...
";
        ...
        assert!(r.greek_letter_ratio > 0.9,
                "expected dominant greek, got {}", r.greek_letter_ratio);
    }
