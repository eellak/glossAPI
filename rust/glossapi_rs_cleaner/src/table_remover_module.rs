...
pub fn remove_tables_from_content(
    ...
    if original_lines.is_empty() {
        // All lines were part of ranges, or original was empty.
        ...
        if !table_locations_for_file.is_empty()
            && all_original_lines_are_whitespace
            && original_lines.len()
                <= table_locations_for_file
                    .iter()
        ...
        if file_content.chars().all(char::is_whitespace) && !table_locations_for_file.is_empty() {
            ...
            let max_end_line = table_locations_for_file
                .iter()
                .map(|r| r.end_line)
                .max()
                .unwrap_or(0);
            if max_end_line >= original_lines.len().saturating_sub(1) && !original_lines.is_empty()
            {
                // Covers all effective lines
                return "<!-- table-removed -->".to_string();
            }
            if original_lines.is_empty() && !file_content.is_empty() {
                // e.g. file_content = "\\n" -> original_lines = [""]
                ...
                if table_locations_for_file
                    .iter()
                    .any(|r| r.start_line == 0 && r.end_line == 0 && original_lines.len() == 1)
                {
                    return "<!-- table-removed -->".to_string();
                }
            }
    ...
    for range in table_locations_for_file {
        for i in
            range.start_line..=std::cmp::min(range.end_line, original_lines.len().saturating_sub(1))
        {
            if i < line_flags.len() {
                line_flags[i] = true;
            }
    ...
    if lines_to_keep.is_empty()
        ...
        && !table_locations_for_file.is_empty()
    {
...
mod tests {
    ...
    fn test_empty_content_with_remove_op() {
        ...
        let ranges = vec![LineRange {
            start_line: 0,
            end_line: 0,
        }]; // Range is technically out of bounds but benign
