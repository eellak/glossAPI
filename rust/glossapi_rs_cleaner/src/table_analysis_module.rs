...
pub fn core_detect_malformed_tables(py: Python, markdown_text: &str) -> PyResult<TableScan> {
    ...
    while i < lines.len() {
        ...
        if TABLE_SEPARATOR_REGEX.is_match(line_content) {
            ...
            if current_separator_line_idx > 0
                && TABLE_ROW_REGEX.is_match(lines[current_separator_line_idx - 1])
            {
            ...
            while next_line_after_separator_idx < lines.len()
                && TABLE_ROW_REGEX.is_match(lines[next_line_after_separator_idx])
            {
                ...
                && TABLE_ROW_REGEX.is_match(lines[current_separator_line_idx - 1])
            {
                ...
                // No header row immediately above
                let issue = Py::new(
                    py,
                    TableIssue::new(
                        current_separator_line_idx + 1, // 1-based line number for separator
            ...
            while body_row_idx <= table_end_line_idx {
                // Iterate through identified body rows
                if body_row_idx < lines.len() {
                    // Ensure we are within bounds (redundant if table_end_line_idx is correct)
                    let row_content = lines[body_row_idx];
                    if TABLE_ROW_REGEX.is_match(row_content) {
                        // Double check, though loop condition should suffice
                        let row_columns = count_table_columns(row_content);
                        if separator_columns > 0 && row_columns != separator_columns {
                            // Only issue if separator had columns
                            let issue = Py::new(
                                py,
                                TableIssue::new(
...
fn count_table_columns(row: &str) -> usize {
    ...
    if trimmed.starts_with('|') && trimmed.ends_with('|') {
        if trimmed.len() <= 1 {
            return 0;
        } // Handle degenerate cases like "|" or "||" -> 0 or 1 col respectively
...
#[pyfunction]
pub fn analyze_tables_in_string(
    py: Python,
    markdown_text: &str,
) -> PyResult<(usize, Vec<Py<TableIssue>>)> {
