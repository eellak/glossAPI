        ...
        match ev {
            ...
            | Event::End(TagEnd::Item) => {
                if let Some(buf) = current.take() {
                    blocks.push(buf.split_whitespace().map(|s| s.to_string()).collect());
                }
            }
    ...
    for ev in Parser::new_ext(md, gfm_options()) {
        match ev {
            ...
            Event::End(TagEnd::TableCell) => {
                if let (Some(buf), Some(row)) = (cur_cell_buf.take(), cur_row.as_mut()) {
                    row.push(buf.split_whitespace().map(|s| s.to_string()).collect());
                }
            }
            Event::Text(t) | Event::Code(t) | Event::Html(t) => {
...
pub fn classify_subsequence_failure(
    ...
    let missing_in_input: Vec<&String> = output.iter().filter(|t| !input_set.contains(t)).collect();
...
pub fn verify_md_structural(input: &str, output: &str) -> MdStructuralReport {
    ...
    let input_canon_owned = strip_cleaner_markers(&canonicalize_for_verify(input));
    ...
    for (i, (a, b)) in par_in.iter().zip(par_out.iter()).enumerate() {
        ...
        if !is_subsequence(b, a) {
            all_pass = false;
            let kind = classify_subsequence_failure(a, b).unwrap_or(SubsequenceFailureKind::Other);
    ...
    if code_pass {
        for (a, b) in code_in.iter().zip(code_out.iter()) {
            if !is_subsequence(b, a) {
                code_pass = false;
                if r.first_diff.is_none() {
                    r.first_diff = Some("code block lines NOT a subsequence".to_string());
                }
                break;
            }
        }
...
#[pyfunction]
pub fn verify_md_structural_py(py: Python<'_>, input: &str, output: &str) -> PyResult<PyObject> {
...
mod tests {
    ...
    fn canonicalization_makes_hr_min_invisible() {
        ...
        let output = "before\n\n---\n\nafter\n";
        let r = verify_md_structural(input, output);
        assert!(r.is_structural_equivalent(), "HR min should pass: {:?}", r);
    }
