...
fn emit_progress_update(reported_percent: &Arc<AtomicUsize>, processed: usize, total: usize) {
    if total == 0 {
        return;
    }
    ...
    loop {
        ...
        let next = std::cmp::min(100, last + 10);
        if percent < next {
            break;
        }
        match reported_percent.compare_exchange(last, next, Ordering::AcqRel, Ordering::Relaxed) {
            Ok(_) => {
                println!(
                    "Rust cleaning progress: {}% ({}/{})",
...
where
    ...
    OpFn: Fn(Python, PathBuf, &str, &Arc<OpConfig>) -> PyResult<Option<PerFileOutput>>
        + Send
        + Sync
        + 'static,
    PerFileOutput: Send + Sync + Clone + 'static,
{
    ...
    if debug_logging {
        eprintln!(
            "glossapi_rs_cleaner: process_directory_core starting on {}",
            input_dir_str
        );
    }
    ...
    if !input_path.is_dir() {
        eprintln!("ERROR: Input path is not a directory: {}", input_dir_str);
    ...
    if let Some(out_p) = output_path_opt {
        if !out_p.exists() {
            if debug_logging {
                eprintln!("INFO: Creating output directory: {}", out_p.display());
            }
    ...
    if debug_logging {
        eprintln!(
            "DEBUG: Collecting markdown files from: {}",
            input_path.display()
        );
    }
    ...
    if debug_logging {
        eprintln!("DEBUG: Found {} markdown files", md_files.len());
    }

    if md_files.is_empty() {
        if debug_logging {
            eprintln!("INFO: No markdown files found in input directory");
        }
        return Ok(Vec::new());
    }
    ...
    if debug_logging {
        eprintln!(
            "DEBUG: Configuring process_directory_core thread pool with {} threads",
            thread_count
        );
    }
    ...
    if debug_logging {
        eprintln!("DEBUG: Starting parallel processing with Rayon");
    }
    ...
    if debug_logging {
        eprintln!("DEBUG: process_directory_core completed processing loop");
    }
    ...
    let final_results = match Arc::try_unwrap(results) {
        Ok(mutex) => mutex
            .into_inner()
            .map_err(|_e| PyValueError::new_err("Mutex for results was poisoned"))?,
        Err(arc_still_shared) => arc_still_shared
            .lock()
            .map_err(|_e| PyValueError::new_err("Mutex for results was poisoned during clone"))?
            .clone(),
    };
...
pub fn generate_analysis_report_for_directory(
    ...
    let (allowed_chars, unusual_chars) =
        cleaning_module::build_script_char_sets(&scripts_to_keep);
    ...
    if md_files.is_empty() {
        ...
        if let Some(csv_path) = output_csv_path_str {
            if !csv_path.is_empty() {
                if let Some(parent_dir) = Path::new(csv_path).parent() {
                    if !parent_dir.as_os_str().is_empty() && !parent_dir.exists() {
                        fs::create_dir_all(parent_dir).map_err(|e| {
                            PyValueError::new_err(format!(
                                "Failed to create parent for empty CSV {}: {}",
                                parent_dir.display(),
                                e
                            ))
                        })?;
                ...
                let mut wtr = Writer::from_path(csv_path).map_err(|e| {
                    PyValueError::new_err(format!(
                        "Failed to create CSV for empty report {}: {}",
                        csv_path, e
                    ))
                })?;
                wtr.write_record([
                ...
                .map_err(|e| {
                    PyValueError::new_err(format!("CSV header write error for empty report: {}", e))
                })?;
                wtr.flush().map_err(|e| {
                    PyValueError::new_err(format!("CSV flush error for empty report: {}", e))
                })?;
            }
        }
    ...
    let collected_report_data_arc: Arc<Mutex<Vec<FileReportData>>> =
    ...
    py.allow_threads(|| {
        pool.install(|| {
            md_files.par_iter().for_each(|md_file_path| {
                ...
                let reported_percent = Arc::clone(&reported_percent);

                if debug_logging {
                    println!("Processing file: {}", md_file_path.display());
                }
                ...
                let file_content = match fs::read_to_string(md_file_path) {
                    ...
                    Err(e) => {
                        ...
                        let processed = progress_counter
                            .fetch_add(1, Ordering::Relaxed)
                            .saturating_add(1);
                        emit_progress_update(&reported_percent, processed, total_files);
                ...
                let removed_total_chars = analysis_result
                    .original_total_chars
                    .saturating_sub(analysis_result.cleaned_total_chars);

                let percentage_greek_cleaned: Option<f64> = analysis_result
                    .greek_char_count_after_clean
                    .and_then(|greek_count| {
                        analysis_result
                            .cleaned_non_whitespace_chars_after_clean
                            .map(|total_script_chars| {
                ...
                let percentage_latin_cleaned: Option<f64> = analysis_result
                    .latin_char_count_after_clean
                    .and_then(|latin_count| {
                        analysis_result
                            .cleaned_non_whitespace_chars_after_clean
                            .map(|total_script_chars| {
                ...
                if let Some(output_base_path) = &*output_cleaned_path_opt_arc {
                    ...
                    if let Err(e) =
                        fs::write(&target_file_path, &analysis_result.cleaned_text_content)
                    {
                        eprintln!(
                            "ERROR: Failed to write cleaned file {}: {}",
                ...
                let report_entry = FileReportData {
                    ...
                    cleaned_non_whitespace_chars: analysis_result
                        .cleaned_non_whitespace_chars_after_clean,
    ...
    println!(
        "Parallel analysis phase completed in: {:.2?}",
        analysis_phase_duration
    );

    let mut collected_report_data = Arc::try_unwrap(collected_report_data_arc)
        .map_err(|_e| {
            PyValueError::new_err("Mutex for report data was poisoned (arc unwrap failed)")
        })?
        .into_inner()
        .map_err(|_e| {
            PyValueError::new_err("Mutex for report data was poisoned (into_inner failed)")
        })?;
    ...
    if let Some(csv_path) = output_csv_path_str {
        if !csv_path.is_empty() {
            ...
            if let Some(parent_dir) = Path::new(csv_path).parent() {
                if !parent_dir.as_os_str().is_empty() && !parent_dir.exists() {
                    // Check parent is not empty string
                    fs::create_dir_all(parent_dir).map_err(|e| {
                        PyValueError::new_err(format!(
            ...
            match Writer::from_path(csv_path) {
                Ok(mut wtr) => {
                    ...
                    for report_item in &collected_report_data {
                        ...
                        .map_err(|e| {
                            PyValueError::new_err(format!(
                                "CSV row write error for {}: {}",
                                report_item.file_name, e
                            ))
                        })?;
                    }
                    wtr.flush()
                        .map_err(|e| PyValueError::new_err(format!("CSV flush error: {}", e)))?;
                    let csv_writing_duration = csv_writing_start.elapsed();
                    println!(
                        "CSV writing phase completed in: {:.2?}",
                        csv_writing_duration
                    );
                }
                Err(e) => {
                    return Err(PyValueError::new_err(format!(
                        "Failed to create CSV writer for path '{}': {}",
                        csv_path, e
                    )));
                }
            }
    ...
    summary.set_item(
        "files_with_potential_read_errors",
        files_with_potential_read_errors,
    )?; // Clarified name
    Ok(summary.into())
}
...
pub fn batch_clean_markdown_files(
    ...
    // Debug prints
    println!(
        "INFO: Starting batch_clean_markdown_files with {} threads",
        num_threads
    );
    ...
    let clean_file_op = move |_py_thread: Python,
                              md_file_path: PathBuf,
                              content: &str,
                              op_conf: &Arc<BatchCleanOpConfig>|
          -> PyResult<Option<PerFileOperationOutput>> {
        ...
        let relative_path = md_file_path
            .strip_prefix(&input_base_path) // Use cloned input_base_path
            .unwrap_or(&md_file_path);
        ...
        fs::write(&target_file_path, cleaned_content_tuple.0).map_err(|e| {
            PyValueError::new_err(format!(
                "Failed to write cleaned file {}: {}",
                target_file_path.display(),
                e
            ))
        })?;

        Ok(Some(PerFileOperationOutput::OperationSuccess(
            md_file_path.clone(),
        )))
    };

    // Process the directory using the core function
    let results: Vec<PerFileOperationOutput> =
        process_directory_core::<BatchCleanOpConfig, _, PerFileOperationOutput>(
            py,
            input_dir,
            Some(output_dir),
            num_threads,
    ...
    summary.set_item(
        "message",
        format!(
            "Batch cleaning completed. {} files processed. See output directory.",
            files_processed_count
        ),
    )?;
...
pub fn batch_generate_table_summary_csv(
    ...
    if md_files.is_empty() {
        ...
        wtr.flush()
            .map_err(|e| PyValueError::new_err(format!("Failed to flush CSV: {}", e)))?;
        return Ok(());
    }
        ...
        pool.install(|| {
            md_files
                ...
                .filter_map(|md_file_path| {
                    Python::with_gil(|py_thread_token| match fs::read_to_string(md_file_path) {
                        Ok(content) => {
                            match table_analysis_module::analyze_table_file_op(
                                py_thread_token,
                                &content,
                            ) {
                                Ok(scan_result) => {
    ...
    let mut wtr = Writer::from_path(output_csv_path_str).map_err(|e| {
        PyValueError::new_err(format!(
            "Failed to create CSV writer for {}: {}",
            output_csv_path_str, e
        ))
    })?;
    ...
    println!(
        "Table summary CSV report generated at: {}",
        output_csv_path_str
    );
    Ok(())
}
...
pub fn batch_generate_detailed_table_report_csv(
    ...
    // Return PyDict summary
    println!(
        "Starting detailed table report generation for CSV: {}",
        output_csv_path_str
    );
    ...
    let operation_fn = move |py_token: Python,
                             file_path: PathBuf,
                             content: &str,
                             _config: &Arc<()>|
          -> PyResult<Option<PerFileOperationOutput>> {
        let relative_path_str = path_to_str(&file_path);
        match table_analysis_module::analyze_table_file_op(py_token, content) {
            // Call analyze_table_file_op
            Ok(scan_result) => {
                let issues_for_file: Vec<DetailedTableIssueReportEntry> = scan_result
                    .issues
                    .into_iter()
                    .map(|issue_obj| {
                ...
                if issues_for_file.is_empty() {
                    ...
                    Ok(Some(PerFileOperationOutput::DetailedTableIssues(
                        issues_for_file,
                    )))
                }
            }
        ...
        if let PerFileOperationOutput::DetailedTableIssues(entries) = file_results_enum {
            ...
            for entry in entries {
                wtr.serialize(entry).map_err(|e| {
                    PyValueError::new_err(format!(
                        "Failed to serialize detailed issue to CSV: {}",
                        e
                    ))
                })?;
    ...
    wtr.flush().map_err(|e| {
        PyValueError::new_err(format!(
            "Failed to flush CSV writer for {}: {}",
            output_csv_path_str, e
        ))
    })?;
    println!(
        "Detailed table report generation complete. Total issues found: {}, Files with issues: {}",
        total_issues_found, files_with_issues
    );
...
pub fn batch_remove_tables_from_files(
    ...
    let table_locations_map = table_remover_module::load_table_locations_from_csv(
        detailed_report_csv_path_str,
    )
    .map_err(|e| {
        PyValueError::new_err(format!(
            "Failed to load table locations from CSV {}: {}",
            detailed_report_csv_path_str, e
        ))
    })?;
    ...
    let file_op = move |_py_thread: Python,
                        md_file_path: PathBuf,
                        content: &str,
                        config: &Arc<HashMap<PathBuf, Vec<table_remover_module::LineRange>>>|
          -> PyResult<Option<()>> {
        // Determine relative path for looking up in map and for output path
        let relative_path = md_file_path
            .strip_prefix(&input_base_path)
            ...
            if !parent_dir.exists() {
                fs::create_dir_all(parent_dir).map_err(|e| {
                    PyValueError::new_err(format!(
                        ...
                        target_output_file_path.display(),
                        e
                    ))
                })?;
            }
        ...
        if let Some(locations_for_this_file) = config.get(&md_file_path) {
            if !locations_for_this_file.is_empty() {
                let modified_content = table_remover_module::remove_tables_from_content(
                    content,
    ...
    let _: Vec<()> = process_directory_core(
        // Result type is Vec<() because file_op returns Option<()
        py,
