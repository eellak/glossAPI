...
lazy_static! {
    ...
    /// URL-ish raw token detector used to avoid normalizing punctuation
    /// runs inside links / host paths.
    pub static ref URL_LIKE_TOKEN_REGEX: Regex =
        Regex::new(r"(?i)^(?:[a-z][a-z0-9+.-]*://\S+|www\.\S+|[A-Za-z0-9-]+(?:\.[A-Za-z0-9-]+)+/\S*)$").unwrap();

    /// Inline HTML comments are intentional placeholders in this corpus
    /// (`<!-- image -->`, `<!-- text-missing -->`, etc.). Punctuation
    /// bucketing must not rewrite their `--` delimiters.
    pub static ref HTML_COMMENT_REGEX: Regex = Regex::new(r"<!--.*?-->").unwrap();
...
/// Tiered bucket for run-length normalization (2026-04-28 policy, v3):
///   1, 2    → 1  (single char / accidental double collapse to one)
///   3       → 3  (unchanged — natural prose triple, e.g. ellipsis)
///   4       → 3  (floor, not nearest-neighbour rounding)
///   5..=19  → 5  (medium run — canonical "short leader" form)
///   20      → 20
...
fn is_dot_like(ch: char) -> bool {
    matches!(ch, '.' | '·' | '•' | '‧' | '⋅' | '⋯' | '…')
}

fn dot_weight(ch: char) -> usize {
    if ch == '…' || ch == '⋯' {
        3
    } else {
        1
...
pub(crate) fn span_is_inside_url_like_token(line: &str, start: usize, end: usize) -> bool {
    let (left, right) = token_bounds_around_span(line, start, end);
    let token = &line[left..right];
    URL_LIKE_TOKEN_REGEX.is_match(token)
}
...
fn is_full_line_html_comment(line: &str) -> bool {
    let trimmed = line.trim();
    trimmed.starts_with("<!--") && trimmed.ends_with("-->")
}

fn is_fence_like_line(line: &str) -> bool {
    let trimmed = line.trim_start();
    trimmed.starts_with("```") || trimmed.starts_with("~~~")
}

/// Normalize dot-like runs (`.`, bullets/dot operators, ellipsis) with one
/// logical ladder. U+2026 counts as three dots; output is ASCII dots.
pub fn normalize_dot_and_ellipsis_runs(line: &str) -> Option<String> {
    if is_full_line_html_comment(line) || is_fence_like_line(line) {
        return None;
    }
    if !line.chars().any(is_dot_like) {
        return None;
    }
    let spans: Vec<(usize, char)> = line.char_indices().collect();
    let mut out = String::with_capacity(line.len());
    let mut changed = false;
    let mut i = 0usize;
    while i < spans.len() {
        let (start_byte, ch) = spans[i];
        if is_dot_like(ch) {
            let start = i;
            let mut logical_len = 0usize;
            while i < spans.len() && is_dot_like(spans[i].1) {
                logical_len += dot_weight(spans[i].1);
                i += 1;
            }
            let end_byte = if i < spans.len() {
                spans[i].0
            } else {
                line.len()
            };
            if span_is_inside_html_comment(line, start_byte, end_byte)
                || span_is_inside_url_like_token(line, start_byte, end_byte)
            {
                out.push_str(&line[start_byte..end_byte]);
                continue;
            }
            let bucket = bucket_run_length(logical_len);
            let original: String = spans[start..i].iter().map(|(_, c)| *c).collect();
            if logical_len == 1 {
                out.push_str(&original);
                continue;
            }
            let replacement = ".".repeat(bucket);
...
/// Back-compat wrapper for callers/tests that still refer to dot runs.
pub fn normalize_dot_runs(line: &str) -> Option<String> {
    if !line.chars().any(is_dot_like) {
        return None;
    }
    normalize_dot_and_ellipsis_runs(line)
}
...
fn is_standalone_accent_run(chars: &[char], start: usize, end: usize) -> bool {
...
fn is_atx_heading_hash_run(line: &str, chars: &[char], start: usize, end: usize) -> bool {
    ...
    after.is_none_or(|c| c.is_whitespace()) && line.trim_start().starts_with('#')
}

fn run_char_kind(ch: char) -> Option<char> {
    if is_dash_like(ch) {
        return Some(ch);
    }
    if is_pipe_like(ch) {
...
/// Normalize non-dot punctuation/leader runs using the same floor ladder as
/// dot/whitespace runs, with guards for URL tokens, ATX headings, full-line
/// comments, and fence markers.
pub fn normalize_punctuation_runs(line: &str) -> Option<String> {
    if is_full_line_html_comment(line) || is_fence_like_line(line) {
        return None;
    }
    let chars: Vec<char> = line.chars().collect();
    if !chars.iter().any(|ch| run_char_kind(*ch).is_some()) {
        return None;
    }
    let byte_spans: Vec<(usize, char)> = line.char_indices().collect();
    let mut out = String::with_capacity(line.len());
    let mut changed = false;
    let mut i = 0usize;
    while i < chars.len() {
        let ch = chars[i];
        if run_char_kind(ch).is_some() {
            let start = i;
            ...
            let start_byte = byte_spans[start].0;
            let end_byte = if i < byte_spans.len() {
                byte_spans[i].0
            } else {
                line.len()
            };
            ...
            if ch == '/' && span_is_inside_url_like_token(line, start_byte, end_byte) {
                out.push_str(&line[start_byte..end_byte]);
                continue;
            }
            if ch == '#' && is_atx_heading_hash_run(line, &chars, start, i) {
                out.push_str(&line[start_byte..end_byte]);
                continue;
            }
            if matches!(ch, '΄' | '´' | '`' | '\u{0301}')
                && !is_standalone_accent_run(&chars, start, i)
            {
                out.push_str(&line[start_byte..end_byte]);
                continue;
            }
...
/// was added.
pub fn normalize_escaped_run_chars(line: &str) -> Option<String> {
    if !line.contains('\\') {
        return None;
    }
    const ESCAPED_RUN_CHARS: [u8; 7] = [b'_', b'*', b'-', b'#', b'.', b'=', b'~'];
    ...
    let prefix_end = i;
    // Count repeated `\<char>` pairs. The escaped char must be the same
    // throughout the run.
    let mut pairs = 0usize;
    let escaped_char =
        if i + 1 < bytes.len() && bytes[i] == b'\\' && ESCAPED_RUN_CHARS.contains(&bytes[i + 1]) {
            bytes[i + 1]
        } else {
            return None;
        };
    while i + 1 < bytes.len() && bytes[i] == b'\\' && bytes[i + 1] == escaped_char {
        pairs += 2;
        i += 2;
    }
    if pairs == 0 {
...
pub fn normalize_escaped_underscore_runs(line: &str) -> Option<String> {
    normalize_escaped_run_chars(line)
}
...
mod tests {
    ...
    fn fold_math_italic_latin_letters() {
        ...
        assert_eq!(fold_line("𝑥 + 𝑦 = 𝑧"), Some("x + y = z".to_string()));
    }
    ...
    fn fold_math_alphanumeric_greek_to_regular_greek() {
        ...
        assert_eq!(fold_line("𝛼 + 𝛽 = 𝛾"), Some("α + β = γ".to_string()));
    }
    ...
    fn ellipsis_runs_collapse() {
        ...
        assert_eq!(normalize_ellipsis_runs("………"), Some("…".to_string()));
    ...
    fn bucket_run_length_tiered_v2() {
        ...
        // {4} → 3
        assert_eq!(bucket_run_length(4), 3);
        // {5..=19} → 5
        for n in 5..=19 {
            assert_eq!(bucket_run_length(n), 5, "n={n}");
        }
        assert_eq!(bucket_run_length(20), 20);
        // > 20 → 20
    ...
    fn dot_runs_tiered_v2() {
        ...
        assert_eq!(normalize_dot_runs(&twenty), None);
        // >20 dots → 20
    ...
    #[test]
    fn dot_and_ellipsis_runs_share_one_ladder() {
        assert_eq!(
            normalize_dot_and_ellipsis_runs("x…………y"),
            Some("x.....y".to_string())
        );
        assert_eq!(
            normalize_dot_and_ellipsis_runs("x…..y"),
        ...
        assert_eq!(
            normalize_dot_and_ellipsis_runs("see https://example.org/a....b"),
            None
        );
    }

    #[test]
    fn punctuation_runs_floor_and_guard_markdown_url_contexts() {
        assert_eq!(
            normalize_punctuation_runs("x====y"),
            Some("x===y".to_string())
        );
        assert_eq!(
            normalize_punctuation_runs("########### heading"),
            Some("##### heading".to_string())
        );
        assert_eq!(normalize_punctuation_runs("### heading"), None);
        assert_eq!(
            normalize_punctuation_runs("https://example.org/a////b"),
            None
        );
        assert_eq!(normalize_punctuation_runs("<!-- image -->"), None);
    }

    #[test]
    fn whitespace_runs_tiered_v2_interior_only() {
        // 2 spaces → 1
        assert_eq!(normalize_whitespace_runs("a  b"), Some("a b".to_string()));
        // 3 spaces — unchanged
        ...
        assert_eq!(normalize_whitespace_runs(&twenty), None);
        // 21+ spaces → 20
    ...
    fn whitespace_runs_preserves_leading_indent() {
        ...
        assert_eq!(normalize_whitespace_runs("    def add(x, y):"), None);
        // 8-space indent (nested code) preserved.
        assert_eq!(normalize_whitespace_runs("        return x + y"), None);
        // Leading tab preserved (list-indent convention).
        assert_eq!(normalize_whitespace_runs("\titem"), None);
    ...
    fn drop_low_salvage_pages_handles_no_headers() {
        ...
        let orig_count = count_nonwhitespace_in_range(&original.lines().collect::<Vec<_>>(), 0, 3);
...
pub fn is_residue_mojibake_line(line: &str) -> bool {
    ...
    for ch in line.chars() {
        let cp = ch as u32;
        let is_residue =
            (0x0180..=0x024F).contains(&cp) && !matches!(cp, 0x0218 | 0x0219 | 0x021A | 0x021B);
        if is_residue {
            cur_run += 1;
            if cur_run > max_run {
                max_run = cur_run;
...
mod wave2_tests {
    ...
    fn strip_base64_images_basic_jpeg() {
        let payload = "/9j/4AAQSkZJRgABAQEAyADIAAD".to_string() + &"X".repeat(500); // realistic Docling-blob size
        let input = format!("before ![Image 1](data:image/jpg;base64,{}) after", payload);
        let out = strip_base64_images(&input);
        assert_eq!(
            out, "before <!-- image --> after",
            "expected base64 image stripped, got: {:?}",
            out
        );
    }
    ...
    fn fold_adobe_pua_via_fold_line() {
        assert_eq!(
            fold_line("\u{F061}\u{F062}\u{F063}"),
            Some("αβχ".to_string())
        );
    }
    ...
    #[test]
    fn escaped_markdown_runs_generalize_beyond_underscore() {
        assert_eq!(
            normalize_escaped_run_chars(r"\*\*\*\*\*\*\*\*"),
            Some(r"\*\*\*\*\*".to_string())
        );
        assert_eq!(
            normalize_escaped_run_chars(r"\#\#\#\#"),
