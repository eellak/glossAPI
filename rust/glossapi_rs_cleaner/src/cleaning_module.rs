...
const TABLE_REMOVED_COMMENT: &str = "<!-- table-removed -->";
...
lazy_static! {
    ...
    pub static ref PDF_GLYPH_NAME_REGEX: Regex = Regex::new(
        r"(?i)GLYPH<[^>]{1,200}>|GLYPH\([^)]{1,200}\)|glyph\[[^\]]{1,200}\]|glyph<c=\d+,font=/[^>]+>|<c=\d+,font=/[^>]+>glyph|/[A-Z]{6}\+[A-Z][A-Za-z0-9-]+|/uni[0-9A-Fa-f]{4,6}|/g(?:id)?\d+"
    ).unwrap();
...
fn span_is_inside_html_comment(line: &str, start: usize, end: usize) -> bool {
    COMMENT_REGEX
        .find_iter(line)
        .any(|m| start >= m.start() && end <= m.end())
}

fn token_bounds_around_span(line: &str, start: usize, end: usize) -> (usize, usize) {
    let left = line[..start]
        .rfind(char::is_whitespace)
        .map(|idx| idx + line[idx..].chars().next().map(char::len_utf8).unwrap_or(1))
        .unwrap_or(0);
    let right = line[end..]
...
fn span_text_is_hyphenminus(line: &str, start: usize, end: usize) -> bool {
    line.get(start..end)
        .map(|text| text.eq_ignore_ascii_case("/hyphenminus"))
        .unwrap_or(false)
}

fn hyphenminus_span_is_inside_true_url(line: &str, start: usize, end: usize) -> bool {
    let (left, right) = token_bounds_around_span(line, start, end);
    let token = &line[left..right];
    let token_lower = token.to_ascii_lowercase();
    if token_lower.starts_with("www.") || token_lower.contains("://") {
        return true;
    }

    if let Some(first_slash) = token.find('/') {
        let before_slash = &token[..first_slash];
        return before_slash.contains('.')
            && before_slash.chars().any(|ch| ch.is_ascii_alphabetic());
    }

    false
}

fn glyph_span_is_protected(line: &str, start: usize, end: usize) -> bool {
    if span_is_inside_html_comment(line, start, end) {
        return true;
    }
    if !normalize::span_is_inside_url_like_token(line, start, end) {
        return false;
    }

    // Numeric ranges such as `4.600/hyphenminus5.600` can look like
    // host/path tokens to the broad URL guard. Keep real links protected,
    // but allow these range markers through for normalization.
    if span_text_is_hyphenminus(line, start, end) {
        return hyphenminus_span_is_inside_true_url(line, start, end);
    }

    true
}

fn hyphenminus_replacement(line: &str, start: usize, end: usize) -> Option<&'static str> {
    if !span_text_is_hyphenminus(line, start, end) {
        return None;
    }
    let left_glued = line[..start]
        .chars()
        .next_back()
        .map(|ch| !ch.is_whitespace())
        .unwrap_or(false);
    let right_glued = line[end..]
        .chars()
        .next()
        .map(|ch| !ch.is_whitespace())
        .unwrap_or(false);
...
fn push_unprotected_glyph_spans<I>(line: &str, matches: I, spans: &mut Vec<GlyphSpan>)
where
    I: Iterator<Item = (usize, usize)>,
{
    for (start, end) in matches {
        if !glyph_span_is_protected(line, start, end) {
            spans.push(GlyphSpan {
                start,
                end,
                replacement: hyphenminus_replacement(line, start, end),
            });
        }
    }
}

fn rewrite_non_overlapping_spans(line: &str, spans: &[GlyphSpan]) -> String {
    if spans.is_empty() {
...
fn is_uppercase_glyph_stem(text: &str) -> bool {
    !text.is_empty()
        && text.len() % 5 == 0
        && text
            .as_bytes()
            .chunks_exact(5)
            .all(|chunk| chunk == b"GLYPH")
}

fn display_math_line_has_glyph_residue(line: &str) -> (bool, usize, usize) {
    let rule_a_count = RULE_A_LITERALS_AC
        .find_iter(line)
        .filter(|m| !span_is_inside_html_comment(line, m.start(), m.end()))
        .count();
    let structured_rule_b_count = PDF_GLYPH_NAME_REGEX
        .find_iter(line)
        .filter(|m| !span_is_inside_html_comment(line, m.start(), m.end()))
        .count();
    let bare_glyph_count = BARE_PDF_GLYPH_STEM_REGEX
        .find_iter(line)
        .filter(|m| {
            !span_is_inside_html_comment(line, m.start(), m.end())
                && is_uppercase_glyph_stem(m.as_str())
        })
        .count();
    let rule_b_count = structured_rule_b_count + bare_glyph_count;

    (rule_a_count + rule_b_count > 0, rule_a_count, rule_b_count)
}
...
///     /font residue regex (GLYPH<…>, GLYPH(…), glyph[…],
///     <c=…,font=/…>glyph, font subset, /uniXXXX, /gN).
...
///   - Wave 3 re-introduces one bounded bare-word exception: repeated
///     / standalone `GLYPH`, which real extractors emit without payload.
///
...
fn apply_glyph_span_strip_and_rule_b(line: &str) -> GlyphStripResult {
    // Count rule A and rule B hits BEFORE stripping, but ignore glyph-like
    // substrings inside URL/path tokens (`/g123`, `/uni03B1`, `/space`, etc.).
    // A link token is ordinary prose context, but its path syntax is not PDF
    // extraction residue.
    let mut rule_a_spans: Vec<GlyphSpan> = Vec::new();
    push_unprotected_glyph_spans(
        line,
        RULE_A_LITERALS_AC
            .find_iter(line)
            .map(|m| (m.start(), m.end())),
    ...
    let mut rule_b_spans: Vec<GlyphSpan> = Vec::new();
    push_unprotected_glyph_spans(
        line,
        PDF_GLYPH_NAME_REGEX
            .find_iter(line)
            .map(|m| (m.start(), m.end())),
        &mut rule_b_spans,
    );
    push_unprotected_glyph_spans(
        line,
        BARE_PDF_GLYPH_STEM_REGEX
            .find_iter(line)
            .map(|m| (m.start(), m.end())),
        &mut rule_b_spans,
    );

    let rule_a_count = rule_a_spans.len();
    let rule_b_count = rule_b_spans.len();
    ...
    let mut spans_to_strip = rule_a_spans;
    spans_to_strip.extend(rule_b_spans);
    let stripped = rewrite_non_overlapping_spans(line, &spans_to_strip);
    GlyphStripResult {
        stripped,
        line_drop,
        rule_a_count,
...
fn clean_impossible_noise_inside_code_fence(line: &str) -> Option<String> {
    if !line.contains('\u{00AD}')
        && !line.contains('\u{00B5}')
        && !line.chars().any(|ch| {
            let code = ch as u32;
            (code < 0x20 && ch != '\t' && ch != '\n')
                || code == 0x7F
...
fn normalize_layout_leader_runs(line: &str) -> Option<String> {
    if line.is_empty() || line == TEXT_MISSING_COMMENT || line == TABLE_REMOVED_COMMENT {
        return None;
    }
    // Tiered bucket per normalize.rs: {2}→1, {3,4}→3, {5..=19}→5, {>20}→20.
    // Uniform with the whitespace-run rule.
    normalize::normalize_dot_and_ellipsis_runs(line)
}
...
pub fn core_clean_text_with_stats_with_mode(
    ...
    for (_line_index, line) in text.lines().enumerate() {
        ...
        if in_code_fence {
            let line_chars = line.chars().count();
            let cleaned_code_line = clean_impossible_noise_inside_code_fence(line);
            let line_to_keep = cleaned_code_line.as_deref().unwrap_or(line);
            let kept_chars = line_to_keep.chars().count();
            original_chars_for_badness += line_chars;
            chars_dropped_by_per_char_filter += line_chars.saturating_sub(kept_chars);
            sum_kept_line_content_chars += kept_chars;
            content_chars_kept += kept_chars;
            cleaned_output_string_builder.push_str(line_to_keep);
            cleaned_output_string_builder.push('\n');
            continue;
        }
        ...
        let display_math_context = carry_math_state
            || trimmed_line == "$$"
            || trimmed_line.starts_with("$$")
            || trimmed_line.ends_with("$$");
        if display_math_context {
            let (drop_math_line, math_rule_a_count, math_rule_b_count) =
                display_math_line_has_glyph_residue(&line_after_entity_decoding_str);
            rule_a_match_count += math_rule_a_count as u64;
            rule_b_match_count += math_rule_b_count as u64;
            if drop_math_line {
                let line_chars = line.chars().count();
                original_chars_for_badness += line_chars;
        ...
        if !skip_bad_line_check && normalize::is_residue_mojibake_line(&post_rule_strip) {
        ...
        let removed_from_tags_count =
            strip_tags_custom(&post_rule_strip, &mut line_after_tag_handling_buf);
        ...
        let line_to_write = if is_exclusively_comment {
            ...
            // preserve intentional HTML comment placeholders, so inline-TMC
            // lines normalize too. Order matters:
            //   dot/ellipsis leader (tiered bucket) -> malformed-entity
            //   fallback -> whitespace run -> escaped Markdown run ->
            //   punctuation run.
            ...
            if let Some(n) = normalize::normalize_escaped_run_chars(&s) {
                s = n;
            }
            if let Some(n) = normalize::normalize_punctuation_runs(&s) {
                s = n;
            }
            s
        };
        ...
        chars_dropped_by_normalization += pre_normalize_chars.saturating_sub(post_normalize_chars);
        ...
        if is_exclusively_comment {
            ...
            // (1 space + TMC.len()). Normalize may already have collapsed
            // surrounding whitespace, but the marker itself is preserved.
...
pub fn clean_text_with_stats(
    ...
    dict.set_item(
        "phase_a_dialect_ambiguous_input",
        stats.phase_a_dialect_ambiguous_input,
    )?;
...
mod tests {
    ...
    #[test]
    fn core_clean_text_bare_glyph_word_stripped_as_bounded_exception() {
        // Wave 3: real PDF extractors emit bare GLYPH placeholders, so
        // `GLYPH` is now the narrow exception to the no-bare-words rule.
        let allowed_chars = default_allowed_chars();
        let unusual_chars = SCRIPT_SETS.get("unusual").cloned().unwrap_or_default();
        let input = "some text with GLYPH in the middle\n";
        let (cleaned, _, kept_chars) = core_clean_text(input, &allowed_chars, &unusual_chars, None);
        assert_ne!(cleaned, format!("{LINE_REMOVED_COMMENT}\n"));
        assert!(!cleaned.contains("GLYPH"));
        assert!(cleaned.contains("some text with"));
        assert!(cleaned.contains("in the middle"));
        assert!(kept_chars > 0);
    }

    #[test]
    fn core_clean_text_repeated_bare_glyph_word_stripped_as_same_family() {
        let allowed_chars = default_allowed_chars();
        let unusual_chars = SCRIPT_SETS.get("unusual").cloned().unwrap_or_default();
        let input = "some text with GLYPHGLYPHGLYPH in the middle\n";
        let (cleaned, _, kept_chars) = core_clean_text(input, &allowed_chars, &unusual_chars, None);
        assert_ne!(cleaned, format!("{LINE_REMOVED_COMMENT}\n"));
        assert!(!cleaned.contains("GLYPH"));
        assert!(cleaned.contains("some text with"));
    ...
    #[test]
    fn core_clean_text_drops_glyph_contaminated_display_math() {
        let allowed_chars = default_allowed_chars();
        let unusual_chars = SCRIPT_SETS.get("unusual").cloned().unwrap_or_default();
        let input = "$$M`GLYPHGLYPHGLYPH$gg]GLYPHGLYPHg\"\\IgGLYPH;GLYPHggGLYPH$$\n";
        let (cleaned, stats) =
            core_clean_text_with_stats(input, &allowed_chars, &unusual_chars, None);
        assert!(
            cleaned.contains(LINE_REMOVED_COMMENT),
            "glyph-contaminated display math should be dropped, got {cleaned:?}"
        );
        assert!(!cleaned.contains("GLYPH"), "got {cleaned:?}");
        assert_eq!(stats.lines_dropped_count, 1);
        assert!(stats.rule_b_match_count > 0, "got {stats:?}");
    }

    #[test]
    fn core_clean_text_keeps_clean_display_math() {
        let allowed_chars = default_allowed_chars();
        let unusual_chars = SCRIPT_SETS.get("unusual").cloned().unwrap_or_default();
        let input = "$$x^2 + y^2 = z^2$$\n";
        let (cleaned, stats) =
            core_clean_text_with_stats(input, &allowed_chars, &unusual_chars, None);
        assert!(cleaned.contains("$$x^2 + y^2 = z^2$$"), "got {cleaned:?}");
        assert_eq!(stats.lines_dropped_count, 0);
        assert_eq!(stats.rule_a_match_count, 0);
        assert_eq!(stats.rule_b_match_count, 0);
    }

    #[test]
    fn core_clean_text_does_not_strip_glyph_like_spans_inside_urls_or_comments() {
        let allowed_chars = default_allowed_chars();
        let unusual_chars = SCRIPT_SETS.get("unusual").cloned().unwrap_or_default();
        let input =
            "see https://example.org/GLYPHGLYPH/a/space/g123/uni03B1 and <!-- GLYPH /space -->\n";
        let (cleaned, stats) =
            core_clean_text_with_stats(input, &allowed_chars, &unusual_chars, None);
        assert!(cleaned.contains("https://example.org/GLYPHGLYPH/a/space/g123/uni03B1"));
        assert!(cleaned.contains("<!-- GLYPH /space -->"));
        assert_eq!(stats.rule_a_match_count, 0);
        assert_eq!(stats.rule_b_match_count, 0);
        assert_eq!(stats.lines_dropped_count, 0);
    }
    ...
    #[test]
    fn core_clean_text_normalizes_hyphenminus_ranges_but_keeps_urls() {
        let allowed_chars = default_allowed_chars();
        let unusual_chars = SCRIPT_SETS.get("unusual").cloned().unwrap_or_default();
        let input = "range 4.600/hyphenminus5.600 and 75/hyphenminus77 and θεσμικο/hyphenminusδιοικητικών\n\
see https://example.org/path/hyphenminus/file\n";
        let (cleaned, stats) =
            core_clean_text_with_stats(input, &allowed_chars, &unusual_chars, None);
        assert!(cleaned.contains("4.600-5.600"), "got {cleaned:?}");
        assert!(cleaned.contains("75-77"), "got {cleaned:?}");
        assert!(cleaned.contains("θεσμικο-διοικητικών"), "got {cleaned:?}");
        assert!(cleaned.contains("https://example.org/path/hyphenminus/file"));
        assert_eq!(stats.lines_dropped_count, 0);
        assert_eq!(stats.rule_a_match_count, 3);
    }

    #[test]
    fn core_clean_text_span_strips_structured_glyph_variants() {
        let allowed_chars = default_allowed_chars();
        let unusual_chars = SCRIPT_SETS.get("unusual").cloned().unwrap_or_default();
        let input = "prefix GLYPH(foo) and glyph[bar] suffix\n";
        let (cleaned, _, kept_chars) = core_clean_text(input, &allowed_chars, &unusual_chars, None);
        assert_ne!(cleaned, format!("{LINE_REMOVED_COMMENT}\n"));
    ...
    fn core_clean_text_dense_pdf_font_subsets_line_drop() {
        ...
        let allowed_chars = default_allowed_chars();
        let unusual_chars = SCRIPT_SETS.get("unusual").cloned().unwrap_or_default();
        let input = "/AAAAAA+Tn/BBBBBB+Tn/CCCCCC+Tn/DDDDDD+Tn\
/EEEEEE+Tn/FFFFFF+Tn/GGGGGG+Tn/HHHHHH+Tn\
/IIIIII+Tn/JJJJJJ+Tn/KKKKKK+Tn/LLLLLL+Tn\n";
        let (cleaned, _, _) = core_clean_text(input, &allowed_chars, &unusual_chars, None);
        assert!(
            cleaned.contains(LINE_REMOVED_COMMENT),
            "dense font-subset line should hit the gate, got {:?}",
            cleaned
        );
    }
    ...
    fn core_clean_text_span_strips_ps_glyph_literals_in_prose() {
        ...
        for input in [
            ...
            assert_ne!(
                cleaned,
                format!("{LINE_REMOVED_COMMENT}\n"),
                "rule A literals should not line-drop: {:?} → {:?}",
                input,
                cleaned
            );
            assert!(
                !cleaned.contains("/hyphenminus")
                    && !cleaned.contains("/space")
                    && !cleaned.contains("/period")
                    && !cleaned.contains("/elipsis"),
                "rule A literal should be stripped from {:?} → {:?}",
                input,
                cleaned
            );
        }
    ...
    fn core_clean_text_skips_fenced_code_block() {
        ...
        let fence_block: Vec<&str> = cleaned
            .lines()
    ...
    #[test]
    fn core_clean_text_only_cleans_impossible_noise_inside_code_fence() {
        let allowed_chars = default_allowed_chars();
        let unusual_chars = SCRIPT_SETS.get("unusual").cloned().unwrap_or_default();
        let input = "```\nlet µ = 1;\u{00AD}\u{0007}\n----\n```\n";
        let (cleaned, _, _) = core_clean_text(input, &allowed_chars, &unusual_chars, None);
        assert!(cleaned.contains("let μ = 1;"));
        assert!(!cleaned.contains('\u{00AD}'));
...
";
    ...
    fn assert_accounting_invariant(input: &str, stats: &CleanStats) {
        let input_chars = input.lines().map(|l| l.chars().count()).sum::<usize>();
...
Επίλογος.\n";
        ...
        assert!(
            stats.chars_dropped_by_line_drop > 0,
        ...
        assert!(
            stats.marker_chars_added >= marker_chars,
            "LINE_REMOVED_COMMENT should be in marker_chars_added: {stats:?}"
        );
        assert_eq!(
            stats.marker_chars_passthrough, 0,
...
/uni0307/uni0308/uni0309/uni030A/uni030B/uni030C\n";
        ...
        assert!(cleaned.contains(LINE_REMOVED_COMMENT));
        assert_eq!(
            stats.marker_chars_added,
            LINE_REMOVED_COMMENT.chars().count()
        );
        assert_eq!(stats.marker_chars_passthrough, 0);
    }
    ...
    fn accounting_per_char_filter_tracks_unusual_script_strip() {
        ...
        assert!(
            stats.chars_dropped_by_per_char_filter >= 12,
...
Επίλογος.\n";
    ...
    fn accounting_rule_a_span_strip_goes_to_per_char_filter_not_line_drop() {
        ...
        assert_eq!(stats.chars_dropped_by_line_drop, 0);
        assert!(
            stats.chars_dropped_by_per_char_filter >= 18,
...
";
    ...
    fn run_full_cleaner(input: &str) -> String {
        let allowed_chars = default_allowed_chars();
        let unusual_chars = SCRIPT_SETS.get("unusual").cloned().unwrap_or_default();
        let (cleaned, _) = core_clean_text_with_stats(input, &allowed_chars, &unusual_chars, None);
        cleaned
    }
    ...
    fn phase_b_v6_11_nbsp_does_not_fuse_words() {
        ...
        assert!(
            out.contains("Η εργασία") || out.contains("Η\u{00A0}εργασία"),
    ...
    fn phase_b_other_unicode_spaces_also_preserved() {
        ...
        assert!(
            !out.contains("alphabeta"),
            "fusion across thin space: {:?}",
            out
        );
    }
    ...
    fn assert_cleaner_matches_canonicalize(input: &str) {
        ...
        let (cleaned, _stats) = core_clean_text_with_stats(input, &allowed, &unusual, None);
    ...
    fn drift_cleaner_eq_canonicalize_on_plain_prose() {
        assert_cleaner_matches_canonicalize("Η εργασία αυτή έχει σκοπό την περιγραφή.\n");
    }
    ...
    fn drift_cleaner_eq_canonicalize_on_soft_wrapped_paragraph() {
        assert_cleaner_matches_canonicalize("first soft-wrapped\npiece of content\nhere.\n");
    }
