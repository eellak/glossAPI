...
def remove_taged_paragraphs(paragraphs,tags,print = False) :
    ...
    for paragraph in paragraphs :
        if paragraph.startswith(tags) :
            ...
            if paragraph.startswith(('##','[Out:')) and not paragraph.startswith(tags) :
                bibliography_flag = False
                newparagraphs.append(paragraph)
                continue
            if print :
...
catalog_tags = ('## Κατάλογος Πινάκων','## ΚΑΤΑΛΟΓΟΣ ΣΧΗΜΑΤΩΝ','## ΚΑΤΑΛΟΓΟΣ ΕΙΚΟΝΩΝ','## ΚΑΤΑΛΟΓΟΣ ΠΙΝΑΚΩΝ','## Κατάλογος Εικόνων','## Κατάλογος Σχημάτων')
